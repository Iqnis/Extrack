<?php 
    ...
    // Show session-based sweet alert message if set
    ...
    // Set budget limit for alert (e.g., $50)
    $budget_limit = 50;

    // Extract numeric value from the string (removing "$ " and checking the value)
    $budget_left_numeric = floatval(str_replace('$', '', str_replace(' ', '', $budget_left)));

    // Trigger alert if budget left is below the limit
    if (is_numeric($budget_left_numeric) && $budget_left_numeric <= $budget_limit) {
        $_SESSION['swal'] = "<script>Swal.fire('Warning!', 'Your budget is running low!', 'warning');</script>";
    }
?>
