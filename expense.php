<?php  
  class Expense extends Base {
    ...
    // Returns all expense records along with category names
    public function allexpWithCategories($UserId) {
      $stmt = $this->pdo->prepare("
          SELECT expense.*, categories.CategoryName 
          FROM expense 
          LEFT JOIN categories ON expense.category_id = categories.CategoryId
    ...
    public function updateExpense($id, $item, $category, $cost) {
      $stmt = $this->pdo->prepare("UPDATE expense SET Item = :item, category_id = :category, Cost = :cost WHERE ID = :id");
      $stmt->bindParam(":id", $id, PDO::PARAM_INT);
      $stmt->bindParam(":item", $item, PDO::PARAM_STR);
      $stmt->bindParam(":category", $category, PDO::PARAM_INT); // Ensure category ID is an integer
